...
- `/shit:attest REQ-AUTH-001` — Single requirement
- `/shit:attest REQ-AUTH` — All requirements with a prefix (one module)
- `/shit:attest 004` — All requirements in spec module 004
...
- **Prefix** (`REQ-XXX`): Find the module in `specs/2-spec/000-index.md` Module Status Overview table by matching the Prefix column. Read that spec file and extract all requirement IDs matching `^### (REQ-[A-Z]+-[0-9]{3}):`
- **Module number** (`NNN`): Look up the filename in `specs/2-spec/000-index.md` Module Status Overview table by matching the Doc # column. Read that spec file and extract all requirement IDs matching `^### (REQ-[A-Z]+-[0-9]{3}):`
...
For each requirement ID, read its spec block from the appropriate `specs/2-spec/*.md` file. Extract:
...
Search the project's source files for any of these provenance marker patterns:
...
The marker may appear in any line-comment style the language uses. Read the matched files. For small files (<300 lines), read the full file. For larger files, read targeted sections around each provenance marker (50 lines of context).

#### 4. Find Test Files

Search the project's test files for both:

**Test framework tags** (convert `REQ-AUTH-003` → `.reqAUTH003` or the project's normalized form):
...
**Provenance markers in test files** (same patterns as code).
...
Write the attestation report to `specs/4-docs/attestations/attest-{NNN}-{module-name}.md` where NNN is the module number and module-name is the module slug (e.g., `attest-004-auth.md`). For single-requirement attestations, use `attest-{REQ-ID}.md` (e.g., `attest-REQ-AUTH-001.md`). Create the `attestations/` directory if it doesn't exist.
