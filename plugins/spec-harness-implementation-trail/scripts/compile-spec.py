...
2. ~~strikethrough~~ headings (current format)

Run from the adopting project's root. The script expects to find spec modules at
specs/2-spec/NNN-*.md and writes the compiled output to specs/2-spec/compiled/.
"""
...
def compile_spec(spec_dir, output_path=None):
    ...
    if not files:
        print(f"Error: no spec modules found in {spec_dir}", file=sys.stderr)
        print("Expected files matching the pattern NNN-*.md (three-digit prefix).", file=sys.stderr)
        sys.exit(1)
    ...
    for fpath in files:
        ...
        while i < len(lines):
            ...
            # Check for ~~strikethrough~~ heading (current format)
    ...
    # Build output
    out = []
    out.append("# Compiled Specification")
...
if __name__ == "__main__":
    # Default: run from the adopting project root
    project_root = os.getcwd()
    spec_dir = os.path.join(project_root, "specs", "2-spec")

    if not os.path.isdir(spec_dir):
        print(f"Error: spec directory not found at {spec_dir}", file=sys.stderr)
        print("Run this from your project root, or run /shit:init to scaffold the specs/ directory.", file=sys.stderr)
        sys.exit(1)
